...
import {Avatar, Button, Card, Empty, Input, List, Pagination, Space, Typography} from "antd";
  ...
  return (
    <div>
      ...
      <div style={{display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginTop: 8, flexWrap: "wrap"}}>
        <Text type="secondary">{value.length} / {maxCommentLength}</Text>
...
function CommentArea({account, targetType, targetKey, targetOwner, disabled = false, unavailableText = ""}) {
  ...
  const submitReply = () => {
    ...
    const [parentOwner, parentName] = replyTo.split("/");
    setReplySubmitting(true);
    CommentBackend.addComment({targetType, targetKey, parentOwner, parentName, content: trimmedContent})
      .then(res => {
        if (res.status === "ok") {
          setReplyTo("");
          setReplyToOwner("");
